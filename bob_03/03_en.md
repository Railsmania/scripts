...
For this lesson we're going to use an Open Authentication strategy, or "Oauth", to log users in to our appliation. If you're not familiar with Oauth, please check the lesson notes for a thorough explanation.

Today, we're going to use the Omniauth gem and an oauth strategy gem to allow users to authenticate via DigitalOcean's Oauth service. So, let's go ahead and add those gems to our gemfile. At the bottom of our gemfile, we'll add omniauth and omniauth-digitalocean gems, each fixed to a specific version.
...
In app/controllers/application_controller add the a method named 'verify session' and add it to the before_filters. We want this method to redirect to the login page. We can find the route to the login page by opening a terminal and typing:
